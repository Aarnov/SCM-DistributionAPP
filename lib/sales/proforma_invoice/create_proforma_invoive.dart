...
double _calculateTotal() {
  ...
  double productTotal = selectedProducts.fold<double>(0, (sum, item) => sum + ((item['subtotal'] ?? 0).toDouble()));
            ...
            onPressed: () {
              ...
              if (refresh) {
                setState(() {
                  ...
                  proformaNo = "PRO-${DateTime.now().millisecondsSinceEpoch % 100000}";
                });
              }
            },
            child: const Text("OK"),
...
Widget _buildEditableField(int index, String key, String value, TextInputType inputType) {
  return SizedBox(
    width: 80,
    child: TextFormField(
...
void _updateProductCalculations(int index) {
  ...
  // Ensure valid values
  double purchasePriceWithoutGST = (product['purchasePriceWithoutGST'] ?? 0.0).toDouble();
  double sellingPriceWithoutGST = (product['sellingPriceWithoutGST'] ?? 0.0).toDouble(); // User input
  ...
  double purchasePriceWithGST = purchasePriceWithoutGST * (1 + gstPercent / 100);
  ...
  double sellingPriceAfterDiscount = sellingPriceWithoutGST - discountAmountPerUnit;
...
Future<void> _saveProformaInvoice(String billStatus) async {
  ...
  double billAmountWithoutGST = selectedProducts.fold<double>(0, (sum, item) => sum + ((item['subtotal'] ?? 0).toDouble()));
  double gstAmount = selectedProducts.fold<double>(0, (sum, item) => sum + ((item['gstAmount'] ?? 0).toDouble()));
  double additionalCharges = double.tryParse(_additionalChargesController.text) ?? 0.0;
  ...
  try {
    // ✅ Save ProformaMaster
    await FirebaseFirestore.instance.collection('ProformaMaster').doc(proformaNo).set({
          ...
          TextButton(
            onPressed: () {
              ...
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(builder: (context) => const CreateProformaInvoice()),
              );
            },
            child: const Text("OK"),
          ),
...
Expanded(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                     ...
                     TextField(
  controller: _billDescriptionController,  // ✅ Attach controller
  ...
  child: Container(
    ...
    child: TextButton.icon(
      onPressed: () => _saveProformaInvoice("Final"),
      icon: const Icon(Icons.save, color: Colors.black),
      label: const Text("Save Proforma", style: TextStyle(fontWeight: FontWeight.bold)),
    ),
  ),
),
...
Expanded(
  child: Container(
    ...
    child: TextButton.icon(
      onPressed: () => _saveProformaInvoice("Draft"),
...
Card(
  ...
  columns: [
    ...
    DataColumn(label: _buildHeader("Purchase Price Without GST")),
    DataColumn(label: _buildHeader("Selling Price Without GST")),
    DataColumn(label: _buildHeader("MRP with Gst")),
    DataColumn(label: _buildHeader("GST %")),
    DataColumn(label: _buildHeader("Sale Discount")),
    DataColumn(label: _buildHeader("Discount Unit")),

    DataColumn(label: _buildHeader("Purchase Price With GST")),
    DataColumn(label: _buildHeader("Selling Price With GST")),
    ...
    return DataRow(
      color: MaterialStateProperty.resolveWith<Color?>(
        (Set<MaterialState> states) {
          return index.isEven ? Colors.grey.shade100 : null; // Alternate row colors
        },
      ),
      cells: [
        ...
        // Editable Quantity
        DataCell(_buildEditableField(index, "quantity", "${product['quantity'] ?? 1}", TextInputType.number)),

        // Editable Purchase Price Without GST
        DataCell(_buildEditableField(index, "purchasePriceWithoutGST", (product['purchasePriceWithoutGST'] ?? 0.0).toStringAsFixed(2), TextInputType.number)),

        // Editable Selling Price Without GST
        DataCell(_buildEditableField(index, "sellingPriceWithoutGST", (product['sellingPriceWithoutGST'] ?? 0.0).toStringAsFixed(2), TextInputType.number)),

        // Editable MRP With GST
        DataCell(_buildEditableField(index, "mrpWithGST", (product['mrpWithGST'] ?? 0.0).toStringAsFixed(2), TextInputType.number)),

        // Editable GST Percent
        DataCell(_buildEditableField(index, "gstPercent", (product['gstPercent'] ?? 0).toString(), TextInputType.number)),

        // Editable Sale Discount
        DataCell(_buildEditableField(index, "saleDiscount", (product['saleDiscount'] ?? 0.0).toStringAsFixed(2), TextInputType.number)),

        // Editable Discount Unit Dropdown
        DataCell(_buildDropdown(index, "saleDiscountUnit", product['saleDiscountUnit'] ?? "%")),

        // Derived Purchase Price With GST
        DataCell(_buildDerivedValue("₹${( product['purchasePriceWithGST'] ?? 0.0).toStringAsFixed(2)}")),

        // Derived Selling Price With GST
        DataCell(_buildDerivedValue("₹${(product['sellingPriceWithGST'] ?? 0.0).toStringAsFixed(2)}")),

        // Subtotal
        DataCell(_buildDerivedValue("₹${(product['subtotal'] ?? 0.0).toStringAsFixed(2)}")),
